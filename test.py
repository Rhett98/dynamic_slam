...
if __name__ == '__main__':
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        # 利用变换矩阵T_gt将pos1转换到pos2
        padp = torch.ones([flat_img1.shape[0], flat_img1.shape[1], 1])
        hom_p1 = torch.cat([flat_img1.cpu(), padp], dim=2)
        trans_pos1 = torch.mm(T_gt[0].float(), hom_p1[0].transpose(1,0).float())[:-1,:].unsqueeze(0)
        t2 = time.time()
        print("img after:", loss_fn.forward(flat_img2.cpu(), trans_pos1.transpose(2,1)))
